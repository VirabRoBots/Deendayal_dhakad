...
# ============================================================================
# NEW: whole-file (non-windowed) subtitle extraction
# ============================================================================
# Separate, simpler code path from extract_subtitle() above — fetches the
# ENTIRE subtitle track in a single ffmpeg call, no -ss/-t windowing.
# Subtitle files are plain text and small (a full 2hr movie's subs are
# typically 40-150KB), so there's no benefit to chunking them the way
# audio/video streams are chunked. This removes the whole class of bugs
# the windowed approach had: no relative-vs-absolute timestamp ambiguity,
# no reshifting on every seek, no window-boundary refetching.
    ...
    async with lock:
        ...
        if entry.get("tracks") is None:
            try:
                ...
                logging.warning(f"[FullSubtitle] get_subtitle_tracks before extract failed: {e}")
