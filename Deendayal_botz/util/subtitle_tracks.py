...
# Fast windowed subtitles (same style as audio_tracks.py)
...
# MUST match the player page window size
...
def _build_ffmpeg_sub_cmd(internal_url: str, stream_index: int, bucket_start: float, out_path: Path) -> list:
...
async def extract_subtitle(msg_id: int, secure_hash: str, stream_index: int, start_time: float = 0.0):
    """Fast windowed subtitle extraction (same style as audio)."""
