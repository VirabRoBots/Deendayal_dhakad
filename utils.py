...
# Fixed Cinemagoer (avoids Invalid SQLite URL: sqlite://cinemagoer.db)
imdb = None
if Cinemagoer is not None:
    try:
        ...
        try:
            ...
            logger.warning("Cinemagoer unavailable, IMDb disabled: %s", e)
            imdb = None
        ...
        if ENABLE_SHORTLINK:
            for file in files:
                ...
                await bot.send_message(
                    ...
                    reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("📤 Dᴏᴡɴʟᴏᴀᴅ 📥", url=await get_shortlink(chat_id, f"https://telegram.me/{temp.U_NAME}?start=files_{file.file_id}"))]])
                )
        else:
            for file in files:
